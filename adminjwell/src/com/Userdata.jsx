...
const Userdata = () => {
  ...
  const { toast } = useToast()
    ...
    const sendCredit = () =>{
      ...
      setData({...data,credits: updateCredits})
	    ...
	    toast({
          description: "Token has been send successfully !",
        })
        }).catch((err)=>{
            console.log(err)
        })
  ...
  return (
   ...
   <div className='flex flex-col'>
    ...
    <p className='font-semibold poppins py-2'><span className='text-gray-600'>Subscription: </span>  {userdata?.subscription == "3" ? "Bussiness version" : userdata?.subscription == null ? "No subscription purchased" : userdata?.subscription}</p>
